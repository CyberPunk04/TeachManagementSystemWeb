...
public class StudentController {
    ...
    public DataResponse getClassmates(@Valid @RequestBody DataRequest dataRequest) {
        String className = dataRequest.getString("className");
        List<Student> sList = studentRepository.findStudentListByClassName(className);
    ...
    public DataResponse getStudentInfo(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if (studentId != null) {
            ...
            op = studentRepository.findByPersonPersonId(personId);
            if(op.isPresent()){
                s = op.get();
            }
        }
    ...
    public DataResponse getStudentIntroduceData(@Valid @RequestBody DataRequest dataRequest) {
        ...
        data.put("studentActivityNum",getStudentActivityNum(s.getStudentId()));
    ...
    public List getStudentActivityNum(Integer studentId){

        List<Prize> alist = prizeRepository.findByStudentStudentId(studentId);
        List<Internship> blist = internshipRepository.findByStudentId(studentId);
        List<Practice> clist = practiceRepository.findByStudentId(studentId);
        List<Research> dlist = researchRepository.findByStudentId(studentId);
        List<Innovation> elist = innovationRepository.findByStudentId(studentId);
        List<Competition> flist = competitionRepository.findByStudentId(studentId);
        List list = new ArrayList();
        Map m;
        m = new HashMap();
        m.put("title","所获荣誉");
        m.put("value",alist.size());
        list.add(m);
        m = new HashMap();
        m.put("title","实习经历");
        m.put("value",blist.size());
        list.add(m);
        m = new HashMap();
        m.put("title","社会实践");
        m.put("value",clist.size());
        list.add(m);
        m = new HashMap();
        m.put("title","科研经历");
        m.put("value",dlist.size());
        list.add(m);
        m = new HashMap();
        m.put("title","创新项目");
        m.put("value",elist.size());
        list.add(m);
        m = new HashMap();
        m.put("title","竞赛经历");
        m.put("value",flist.size());
        list.add(m);
        return list;
    }
