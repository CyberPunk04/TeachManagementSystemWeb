...
public class ScoreController {
    ...
    public DataResponse scoreSave(@Valid @RequestBody DataRequest dataRequest) {
        ...
        Optional<Score> op;
        Score s = null;
            op= scoreRepository.findByCourseStudent(studentId,courseId);
            if(op.isPresent()) {
                s = op.get();
            }
        if(s == null) {
    ...
    @PostMapping("/scoreNoSave")
    public DataResponse scoreNoSave(@Valid @RequestBody DataRequest dataRequest) {
        Integer studentId = dataRequest.getInteger("studentId");
        Integer courseId = dataRequest.getInteger("courseId");
        Integer mark = dataRequest.getInteger("mark");
        Integer scoreId = dataRequest.getInteger("scoreId");
        ...
        op= scoreRepository.findByCourseStudent(studentId,courseId);
        if(op.isPresent()) {
            return CommonMethod.getReturnMessageError("该学生该课程分数已存在");
        }
        if(s == null) {
            s = new Score();
            s.setStudent(studentRepository.findById(studentId).get());
