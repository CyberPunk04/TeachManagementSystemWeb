...
public class BaseController {
    ...
    public DataResponse updatePassword(@Valid @RequestBody DataRequest dataRequest) {
        ...
        String code = dataRequest.getString("code");  //获取验证码
    ...
    //处理邮箱业务
    @PostMapping("/updatePassword/emailVerify")
    public DataResponse updatePasswordEmailVerify(@Valid @RequestBody DataRequest dataRequest) throws MessagingException {
        ...
        emailService.sendTextEmail(email,"【SDU】修改密码","您好，\n" +
                "\n" +
                "您的SDU密码有人企图更改。如果是您进行的更改，请输入下方验证码。 \n" +
                "\n" +
                "如果您未更改密码，其他人则可能已获得您的账户访问权限。请及时重置密码。\n验证码为："+rightCode+"，请在5分钟内完成验证。");
        return CommonMethod.getReturnMessageOK();
    }
    ...
    @PreAuthorize(" hasRole('ADMIN') or  hasRole('STUDENT') or  hasRole('TEACHER')")
    public DataResponse updateEmail(@Valid@RequestBody DataRequest dataRequest){
        String oldEmail = dataRequest.getString("oldEmail");
        String newEmail = dataRequest.getString("newEmail");
        String code = dataRequest.getString("code");
        if(!code.equals(rightCode)){
            return CommonMethod.getReturnMessageError("验证码错误！");
        }
        ...
        if (!oldEmail.equals(p.getEmail())) {
            return CommonMethod.getReturnMessageError("原始邮箱不正确！");
        }
        p.setEmail(newEmail);
        u.setPerson(p);
    ...
    //处理邮箱业务
    @PostMapping("/updateEmail/emailVerify")
    public DataResponse updatEmailEmailVerify(@Valid @RequestBody DataRequest dataRequest) throws MessagingException {
        String oldEmail = dataRequest.getString("oldEmail");
        String newEmail = dataRequest.getString("newEmail");
        ...
        //处理邮箱业务
        rightCode= (Math.random()+"").substring(2,8);
        emailService.sendTextEmail(oldEmail,"【SDU】修改密码","您好，\n" +
                "\n" +
                "您的SDU账号信息有人企图更改。如果是您进行的更改，请输入下方验证码。 \n" +
                "\n" +
                "如果您未尝试更改信息，其他人则可能已获得您的账户访问权限。请及时重置密码。");

        emailService.sendTextEmail(newEmail,"【SDU 更换邮箱","您好，\n" +
                "\n" +
                "\n验证码为："+rightCode+"，请在5分钟内完成验证。");
        return CommonMethod.getReturnMessageOK();
    }
