  ...
  <div class="base_form">
    ...
    <div class="base_query_oneLine">
      <div class="query_left">
        <el-button class="commButton" @click="addRow()">添加新成员</el-button>
      </div>
    </div>
    <!-- 数据表区 -->
    <div class="table-content">
      <el-table :data="dataList" :header-cell-style="{
        ...
        <el-table-column label="关系" width="220" color="black" align="center">
          <template v-slot="scope">
            <el-input v-model="scope.row.relation" style="width: 90%" />
          </template>
        </el-table-column>
        <el-table-column label="姓名" width="220" color="black" align="center">
          <template v-slot="scope">
            <el-input v-model="scope.row.name" style="width: 90%" />
          </template>
        </el-table-column>
        <el-table-column label="性别" width="220" color="black" align="center">
          <template v-slot="scope">
        ...
        <el-table-column label="年龄" width="160" color="black" align="center">
          <template v-slot="scope">
            <el-input v-model="scope.row.age" style="width: 90%" />
          </template>
        </el-table-column>
        <el-table-column label="单位" width="220" color="black" align="center">
          <template v-slot="scope">
            <el-input v-model="scope.row.unit" style="width: 97%" />
          </template>
        </el-table-column>
        <el-table-column label="操作" width="220" color="black" align="center">
          <template v-slot="scope">
...
export default defineComponent({
  ...
  methods: {
    ...
    async saveItem(index: number) {
      ...
      if (
        item.relation == null || item.relation == '' || item.relation == undefined
        || item.name == null || item.name == '' || item.name == undefined
        || item.gender == null || item.gender == '' || item.gender == undefined
        || item.age == null || item.age == undefined || isNaN(item.age) || item.age > 130 || item.age <= 0 ||
        item.unit == null || item.unit == '' || item.unit == undefined) {
        message(this, '请输入完整数据，并检查年龄是否输入合法')
