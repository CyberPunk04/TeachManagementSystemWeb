<template>
    <div class="base_form">
        ...
        <div class="base_query_oneLine">
            <div class="query_left">
                <el-button round style="font-size: 18px" @click="choose()">查看已选课程</el-button>
            </div>
        ...
        <div class="table-content">
            <el-table
                ...
                <el-table-column label="剩余容量" color="black" align="center">
                        <template v-slot="scope">
                            {{ scope.row.last }}
                        </template>
                    </el-table-column>
                ...
                <el-table-column label="操作" color="black" align="center">
                    <template v-slot="scope">
                        <el-button class="commButton" @click="editItem(scope.row)">选课</el-button>
                        <el-button class="commButton" @click="deleteItem(scope.row.courseId)">退选</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
    ...
    <el-dialog title="课程添加修改" :width="350" :height="500" :before-close="handleClose" v-model="dialogVisible"
        ...
        <el-form :model="form" label-width="80px">
            ...
            <el-form-item label="学分">
                <el-input v-model="form.credit" :disabled="true" />
            </el-form-item>
        </el-form>
        <el-row :gutter="80" class="dialog-buttons">
            ...
            <el-col :span="12">
                <el-button type="primary" @click="confirm">确认选课</el-button>
            </el-col>
        </el-row>
    </el-dialog>

    <el-dialog title="课程时间详情" :width="350" :height="350" :before-close="handleClose" v-model="classTimeVisible"
        :append-to-body='true'>
        <el-form :model="form" label-width="80px">
    ...
    <el-dialog title="已选课程" :before-close="handleClose" v-model="chooseVisible">
        <el-table :data="courseChooseList">
            <el-table-column label="课序号" align="center">
                <template v-slot="scope">
                    {{ scope.row.num }}
                </template>
            </el-table-column>
...
import { getCourseList, getTeacherList, courseDelete, courseSave, getCoursePageData, courseSelectionSave, courseSelectionDelete, getCourseSelectionList } from '~/services/courseServ'
...
export default defineComponent({
    ...
    methods: {
        ...
        async confirm() {
            this.close()
            const res = await courseSelectionSave(this.form)
            if (res.code == 0) {
                message(this, '选课成功')
                this.query()
            } else {
        ...
        async deleteItem(courseId: number) {
            ...
            const res = await courseSelectionDelete(courseId)
            if (res.code == 0) {
                message(this, '退选成功')
                this.query()
            } else {
